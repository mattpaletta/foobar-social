...
class NewsFeedDataAccess : Foobar_NewsFeedDataAccess_NewsFeedDataAccessServiceProvider {
    ...
    init(redis: Connection, psql: Connection) {
        ...
        self.postgres = PostgreSQLConnection(host: psql.host, port: psql.port, options: [
                                                                                        .databaseName("news_feed_data_access"),
                                                                                        .userName("docker"),
                                                                                        .password("password"),
                                                                                        .connectionTimeout(0)])
...
let inst = NewsFeedDataAccess(redis: Connection(host: "news-feed-data-access-redis", port: 6379),
                              psql: Connection(host: "news-feed-data-access-postgres", port: 5432))
